...
function setLoading(show){
  ...
  document.body.querySelector('.body').style.display=show?'none':'block';
}

function showResult(r){
  const c=color(r.score), b=bg(r.score);
  const flags=(r.flags||[]).slice(0,3).map(f=>`<div class="flag-item"><div class="flag-dot" style="background:${c}"></div><div>${f.text||f}</div></div>`).join('');
  ...
  document.getElementById('result').innerHTML=`
    ...
    <a class="full-btn" href="https://cyberwatchai.com" target="_blank">Full scan on CyberWatch AI →</a>
  `;
...
async function doScan(url){
  ...
  try{
    const res=await fetch(API_URL,{
      ...
      body:JSON.stringify({system:SYSTEM_PROMPT,messages:[{role:'user',content:`Analyse this URL: ${url}`}]})
    });
    clearInterval(iv);
...
chrome.tabs.query({active:true,currentWindow:true},tabs=>{
  ...
  if(url&&!url.startsWith('chrome://'))document.getElementById('url-input').value=url;
});
